...
- **Current branch reconciliation — PUX-12A / ADR 0007 (2026-09-23):** TEMP-R1 was Human Integrated through PR #51 at merge commit `d6b7df1bf833e0be806df77873ed4de9695ccc5a`. Accepted FE-D1–FE-D6 are recorded in [ADR 0007](./decisions/0007-frontend-ranked-queue-ux.md). This branch implements the ranked frontend queue through `GET /api/v1/assessments` when analytics is ready, preserves manual single-batch lookup, and has not yet passed Human Integration. Production deployment is not claimed.
...
| **Frontend Consumption** | `IMPLEMENTED IN PUX-12A CHANGESET` | [`frontend/src/api/client.ts`](../frontend/src/api/client.ts), [`frontend/src/pages/HomePage.tsx`](../frontend/src/pages/HomePage.tsx), [`frontend/src/components/AssessmentQueue.tsx`](../frontend/src/components/AssessmentQueue.tsx) | React shell handles health, queue, detail, and manual lookup states. When analytics is ready, it calls `GET /api/v1/assessments` and renders the backend-ordered collection; queue selection reuses the returned `RiskAssessment`. Manual `GET /api/v1/assessments/{batch_id}` retains 404/409/503 feedback (PUX-11A / PR #46). | Vladimir (PUX-12A) / prscr (UX/Demo/QA) | Operator, Demo presenter | Human Integration review of this branch implementation. | Hardcoding mock numbers that bypass backend responses, failing to handle unavailable/error states. |
...
- **Future planned output:** Later gates for unresolved engine, calibration, explanation and action policies; VLD-03 integration verification, VLD-04 operational runbook, and VLD-05 claim audit. ADR 0003 accepted task/target, baseline/evaluation and output policies; ADR 0005 defined initial serving policy, fulfilled by RBS-01. Backend collection serving is fulfilled by IGR-05B; PUX-12A implements frontend queue presentation in this branch. Future learned engine, engine minimums beyond the current baseline, production training/retraining and deployment remain unresolved/deferred.
- **Who consumes output:** All workstream owners (Igor, Viktor, prscr, Alisa).
- **Downstream work blocked until evidence exists:** Decision gates are task-specific. Canonical predictive-input semantics (ADR 0002) and canonical mapping (IGR-03) are complete. ADR 0003 accepts bounded target/evaluation/baseline and output semantics. ADR 0004 accepts MVP product scope decisions APR2-D1–D6. PUX-08 is integrated as DRAFT FOR REVIEW (governed by ADR 0004 where applicable); APR-02 source documents remain mixed research/spec artifacts. IGR-04A/04B and RBS-01 baseline serving (PR #42), PUX-11A single-batch frontend lookup (PR #46), and IGR-05B backend collection serving (PR #49) are implemented. PUX-12A frontend collection queue/presentation is implemented in this branch, pending Human Integration; deployment follows separately. No production learned engine is selected or required. New backend model or frontend claims still require their specific evidence and decisions.
...
- **Future planned output:** Backend single-batch and collection serving are complete. Frontend collection queue is implemented in the PUX-12A branch pending Human Integration; learned/explanation/recommendation/deterioration work and deployment remain separately gated or deferred.
- **Who consumes output:** prscr (API responses for UI consumption), Vladimir (system coherence and test verification).
- **Downstream work blocked until evidence exists:** RBS-01 single-batch serving, PUX-11A frontend real-route lookup, and IGR-05B backend collection serving are complete and integrated. The PUX-12A frontend ranked queue is implemented in this branch pending Human Integration. Learned models, recommendations, deterioration prediction, and new engine minimum policies remain subject to their respective decisions/evidence.
...
- **Current PUX-11A and PUX-12A output:** PR #46 integrated real single-batch assessment lookup through `getAssessment(batchId)` and `GET /api/v1/assessments/{batch_id}`. PUX-12A implements ranked collection consumption and queue presentation in this branch under accepted ADR 0007; Human Integration is pending.
- **Accepted output that currently exists:** [`prscr_ux_demo_qa_report.md`](../prscr_ux_demo_qa_report.md) (PUX-00–07, PR #9, repository root); React+TS+Vite shell (`frontend/src/`), connection state management (`HomePage.tsx`), assessment rendering (`AssessmentCard.tsx`), health status display (`BackendStatus.tsx`), typed API client (`api/client.ts`, `api/contracts.ts`), workstream contract [`docs/workstreams/prscr-ux-demo-qa.md`](./workstreams/prscr-ux-demo-qa.md); PUX-08 data-to-UX reconciliation report ([`docs/recon/PUX-08-data-ux-reconciliation.md`](./recon/PUX-08-data-ux-reconciliation.md), PR #22), integrated as **`DRAFT FOR REVIEW / NOT CANON`**. Acceptance of recon artifacts does not make their provisional wireframes, API proposals, or candidate workflows accepted decisions, or their planned QA cases executed tests. PUX-12A branch output is identified separately above because it is not yet Human Integrated.
...
- **Downstream work blocked until evidence exists:** Single-batch real lookup is integrated, the backend collection endpoint is available, and the frontend ranked queue is implemented in this branch. Human Integration of PUX-12A remains pending. Additional batch-selection tables, risk meters, and recommendation widgets remain separate UX work under accepted product scope (ADR 0004) and their own evidence or decision prerequisites. PUX-08 remains draft evidence.
...
| **B6: API → Frontend** | API endpoints | React UI client (`frontend/src/api/client.ts`, `contracts.ts`) | TypeScript interfaces mirror Pydantic assessment and collection models; fetch client handles collection and single-batch requests. | **PUX-12A IMPLEMENTED IN CHANGESET:** the queue consumes `GET /api/v1/assessments` when analytics is ready, preserves backend order, and selects collection objects directly for detail. PUX-11A manual lookup still uses `GET /api/v1/assessments/{batch_id}`. | Realtime source | Vladimir (`PUX-12A`), prscr (`PUX-11A`) & Alisa (`APR-01`) | Human Integration review of the queue implementation. | Vladimir (VLD-02 / VLD-03 Gate) with prscr & Igor | Manual divergence between TypeScript interfaces and Python Pydantic models; UI crashing on missing optional fields; building workflows disconnected from operator realities. |
...
The initial runtime slice (offline artifact generation/validation, runtime loading, lifespan/provider, single-batch GET, health reconciliation and focused verification) is implemented in code and integrated via RBS-01 / PR #42. PUX-11A / PR #46 implemented frontend consumption of the real single-batch route. IGR-05B / PR #49 implemented backend multi-batch/replay serving, facility filtering, ranking and pagination. TEMP-R1 / PR #51 integrated UTC+03 source-time semantics. PUX-12A implements frontend collection queue/presentation in this branch; database, learned/recommendation/deterioration engines, new confidence/minimum policy, comparability, background retraining, hot reload, deployment and VLD-03 remain deferred. If a valid canonical input cannot legitimately be assessed by the existing baseline: **STOP — HUMAN DECISION REQUIRED**. Dataset-backed simulation is not the synthetic fixture; fixture semantics remain unchanged and simulation=false remains outside HG-R5.
...
| PUX-12A / ADR 0007 | ACCEPTED HUMAN DECISION / IMPLEMENTED IN CHANGESET | [ADR 0007](./decisions/0007-frontend-ranked-queue-ux.md) | FE-D1–FE-D6 accepted 2026-09-23; current branch implementation pending Human Integration |
...
| PUX-12A / ADR 0007 | **ACCEPTED HUMAN DECISION / IMPLEMENTED IN CHANGESET** | Queue-first frontend consumes `GET /api/v1/assessments` under FE-D1–FE-D6; Human Integration remains a separate gate |
...
3. **Single-Batch Frontend Integrated; Ranked Queue Implemented in PUX-12A Branch:** PUX-11A (PR #46) integrated real single-batch assessment lookup through `GET /api/v1/assessments/{batch_id}` with HTTP 404/409/503 feedback. PUX-12A consumes `GET /api/v1/assessments` when analytics is ready, renders the backend-ordered replay queue, and reuses the selected collection assessment for detail. Its Human Integration is pending.
