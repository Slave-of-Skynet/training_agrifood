...
- **Historical reconciliation — VLD-R8 / IGR05-D2R (2026-09-23):** PUX-11A integrated real single-batch frontend lookup via PR #46; IGR-05B implemented the backend collection route and shared collection response contract and was accepted / Human Integrated via PR #49. IGR05-D2R corrects only the omitted-bounds default to Nov 29–Dec 1 for the pinned snapshot. The ranked frontend queue was still unimplemented at that point.
- **Current reconciliation — TEMP-R1 / ADR 0006 (2026-09-23):** Accepted Human Gate decisions TEMP-D1–D5 correct collection-serving timestamp semantics to use the official continuous source-local UTC+03 clock (`sponsor_pack/README.md` §4). Naive source/API datetimes normalize to UTC+03 without wall-clock shift; aware API boundaries normalize to UTC+03 preserving physical instant; effective response boundaries expose `+03:00`. The D2R default window is `2025-11-29T00:00:00+03:00 <= dispatch_datetime < 2025-12-01T00:00:00+03:00` (48 hours), preserving the same pinned 18-batch cohort.
- **Current committed state — PUX-12A / ADR 0007 (2026-09-23):** TEMP-R1 was Human Integrated through PR #51 at merge commit `d6b7df1bf833e0be806df77873ed4de9695ccc5a`. Accepted FE-D1–FE-D6 are recorded in [ADR 0007](./decisions/0007-frontend-ranked-queue-ux.md). PUX-12A implemented the ranked frontend queue through `GET /api/v1/assessments` when analytics is ready, preserves manual single-batch lookup, and was accepted / implemented / Human Integrated through [PR #52](https://github.com/Slave-of-Skynet/training_agrifood/pull/52), merge commit `b88e81a700188d9dd7dc95d9be757a6b6b81f900`. Production deployment is not claimed.
- **Historical reconciliation — VLD-03 (2026-09-23):** [End-to-end integration and behaviour evidence](./recon/VLD-03-end-to-end-integration.md) is **ACCEPTED / EVIDENCE COMPLETE / HUMAN INTEGRATED** through [PR #53](https://github.com/Slave-of-Skynet/training_agrifood/pull/53), merge commit `4363e372633eaeb09b9708c31923701852873455`. Real runtime/API/browser checks, synthetic collection fault injection, lookup race, and mobile behavior were verified.
- **Current branch state — VLD-04A (2026-09-23):** [Operational readiness & demo documentation](./recon/VLD-04-runtime-demo-readiness.md) is **EVIDENCE RECORDED / READY FOR HUMAN REPOSITORY GATE** on base commit `4363e372633eaeb09b9708c31923701852873455` (PR target `a177c20dad4ed74be2dd5f46120ecc73767b8dcf`). Clean detached worktree reproducibility, Mode B startup, production-build fallback, demo fallback ladder, and VLD04-D1 decision (Option A: Local-first authoritative demo, accepted by Vladimir) were verified. Parent gate VLD-04 remains **BLOCKED: MISSING EVIDENCE** pending independent teammate reproduction by Igor under VLD-04B.
- **Current committed state — VLD-05A (2026-09-23):** [Pre-final claim & evidence audit](./recon/VLD-05A-claim-evidence-audit.md) was **ACCEPTED / HUMAN INTEGRATED** through [PR #54](https://github.com/Slave-of-Skynet/training_agrifood/pull/54), merge commit `a177c20dad4ed74be2dd5f46120ecc73767b8dcf`. Final VLD-05 remains blocked on final pitch narrative and deck consistency.
...
| **Frontend Consumption** | `ACCEPTED / IMPLEMENTED / HUMAN INTEGRATED (PUX-12A / PR #52)` | [`frontend/src/api/client.ts`](../frontend/src/api/client.ts), [`frontend/src/pages/HomePage.tsx`](../frontend/src/pages/HomePage.tsx), [`frontend/src/components/AssessmentQueue.tsx`](../frontend/src/components/AssessmentQueue.tsx) | React shell handles health, queue, detail, and manual lookup states. When analytics is ready, it calls `GET /api/v1/assessments` and renders the backend-ordered collection; queue selection reuses the returned `RiskAssessment`. Manual `GET /api/v1/assessments/{batch_id}` retains 404/409/503 feedback (PUX-11A / PR #46). | Vladimir (PUX-12A) / prscr (UX/Demo/QA) | Operator, Demo presenter | Human Integrated through PR #52; VLD-03 verifies the end-to-end behavior separately. | Hardcoding mock numbers that bypass backend responses, failing to handle unavailable/error states. |
...
│    ──► PUX-11A single-batch UI ──► ranked queue UI ──► VLD-03          │
│                                                                         │
...
- **Current and future output:** VLD-03 integration was Human Integrated through PR #53. VLD-04A operational demo readiness evidence is recorded in this changeset; VLD04-D1 accepts local-first demo mode; parent VLD-04 remains blocked pending independent teammate reproduction (VLD-04B). VLD-05A claim audit was Human Integrated through PR #54. Later gates remain for unresolved engine, calibration, explanation and action policies, and final VLD-05 claim audit. ADR 0003 accepted task/target, baseline/evaluation and output policies; ADR 0005 defined initial serving policy, fulfilled by RBS-01. Backend collection serving is fulfilled by IGR-05B; PUX-12A frontend queue presentation was Human Integrated through PR #52. Future learned engine, engine minimums beyond the current baseline, production training/retraining and deployment remain unresolved/deferred.
- **Who consumes output:** All workstream owners (Igor, Viktor, prscr, Alisa).
- **Downstream work blocked until evidence exists:** Decision gates are task-specific. Canonical predictive-input semantics (ADR 0002) and canonical mapping (IGR-03) are complete. ADR 0003 accepts bounded target/evaluation/baseline and output semantics. ADR 0004 accepts MVP product scope decisions APR2-D1–D6. PUX-08 is integrated as DRAFT FOR REVIEW (governed by ADR 0004 where applicable); APR-02 source documents remain mixed research/spec artifacts. IGR-04A/04B and RBS-01 baseline serving (PR #42), PUX-11A single-batch frontend lookup (PR #46), and IGR-05B backend collection serving (PR #49) are implemented. PUX-12A frontend collection queue/presentation was Human Integrated through PR #52; deployment follows separately. No production learned engine is selected or required. New backend model or frontend claims still require their specific evidence and decisions.
...
- **Future planned output:** Backend single-batch and collection serving are complete. Frontend collection queue was Human Integrated through PUX-12A / PR #52; learned/explanation/recommendation/deterioration work and deployment remain separately gated or deferred.
- **Who consumes output:** prscr (API responses for UI consumption), Vladimir (system coherence and test verification).
- **Downstream work blocked until evidence exists:** RBS-01 single-batch serving, PUX-11A frontend real-route lookup, and IGR-05B backend collection serving are complete and integrated. The PUX-12A frontend ranked queue was Human Integrated through PR #52. Learned models, recommendations, deterioration prediction, and new engine minimum policies remain subject to their respective decisions/evidence.
...
- **Current PUX-11A and PUX-12A output:** PR #46 integrated real single-batch assessment lookup through `getAssessment(batchId)` and `GET /api/v1/assessments/{batch_id}`. PUX-12A implemented ranked collection consumption and queue presentation under accepted ADR 0007 and was Human Integrated through PR #52.
- **Accepted output that currently exists:** [`prscr_ux_demo_qa_report.md`](../prscr_ux_demo_qa_report.md) (PUX-00–07, PR #9, repository root); React+TS+Vite shell (`frontend/src/`), connection state management (`HomePage.tsx`), assessment rendering (`AssessmentCard.tsx`), health status display (`BackendStatus.tsx`), typed API client (`api/client.ts`, `api/contracts.ts`), workstream contract [`docs/workstreams/prscr-ux-demo-qa.md`](./workstreams/prscr-ux-demo-qa.md); PUX-08 data-to-UX reconciliation report ([`docs/recon/PUX-08-data-ux-reconciliation.md`](./recon/PUX-08-data-ux-reconciliation.md), PR #22), integrated as **`DRAFT FOR REVIEW / NOT CANON`**. Acceptance of recon artifacts does not make their provisional wireframes, API proposals, or candidate workflows accepted decisions, or their planned QA cases executed tests. PUX-12A output was Human Integrated through PR #52.
...
- **Downstream work blocked until evidence exists:** Single-batch real lookup, the backend collection endpoint, and the frontend ranked queue are integrated. PUX-12A was Human Integrated through PR #52. Additional batch-selection tables, risk meters, and recommendation widgets remain separate UX work under accepted product scope (ADR 0004) and their own evidence or decision prerequisites. PUX-08 remains draft evidence.
...
| **B6: API → Frontend** | API endpoints | React UI client (`frontend/src/api/client.ts`, `contracts.ts`) | TypeScript interfaces mirror Pydantic assessment and collection models; fetch client handles collection and single-batch requests. | **PUX-12A ACCEPTED / IMPLEMENTED / HUMAN INTEGRATED (PR #52):** the queue consumes `GET /api/v1/assessments` when analytics is ready, preserves backend order, and selects collection objects directly for detail. PUX-11A manual lookup still uses `GET /api/v1/assessments/{batch_id}`. | Realtime source | Vladimir (`PUX-12A`), prscr (`PUX-11A`) & Alisa (`APR-01`) | Queue implementation Human Integrated through PR #52; VLD-03 verifies integration behavior. | Vladimir (VLD-02 / VLD-03 Gate) with prscr & Igor | Manual divergence between TypeScript interfaces and Python Pydantic models; UI crashing on missing optional fields; building workflows disconnected from operator realities. |
| **B7: Assessment Semantics → Demo** | Application UI & Backend | Demo script & Operator experience | [`docs/demo_runbook.md`](./demo_runbook.md): Verified local startup in Mode B (dataset-backed ranked queue replay), health check, fallback ladder, and explicit simulation boundaries. | **LOCAL DEMO CANDIDATE OPERATIONAL (VLD-04A); BLOCKED ON VLD-04B:** Authoritative demo candidate is the dataset-backed Mode B ranked queue (`GET /api/v1/assessments`) on local host; VLD04-D1 accepted local-first; remote deployment deferred / not required. Parent VLD-04 remains BLOCKED: MISSING EVIDENCE on independent teammate reproduction (VLD-04B). | Hosting constraints, Persistence needs | Vladimir (VLD-04A operational evidence), Igor (VLD-04B independent reproduction) | Reproducible step-by-step runbook; proven fallback for offline/backend failure; clearly labeled simulation boundaries (VLD-04A satisfied); independent teammate reproduction (VLD-04B pending). | Vladimir (VLD-04 Gate) | Presenting synthetic demo data as real prediction results; live demo failure due to unhandled exceptions or network dependencies; claiming production deployment. |
...
| **Hosting constraints** | B7 (Demo) / Runtime Environment | Vladimir (`VLD-04A`), Igor (`VLD-04B`), prscr (Demo needs) | Target demonstration environment needs, network connectivity requirements, hardware resources, deployment complexity. | VLD-04 Gate: VLD04-D1 accepted local-first authoritative demo; remote deployment deferred / not required for the current candidate. | Vladimir: Independent teammate reproduction (VLD-04B). | **VLD04-D1 ACCEPTED (LOCAL-FIRST); VLD-04 BLOCKED ON VLD-04B:** Authoritative demo candidate is local-first in Mode B; remote deployment deferred / not required. Operational runbook and local verification completed under VLD-04A; parent VLD-04 gate blocked only on independent teammate reproduction evidence (VLD-04B). |
...
- **Deployment / hosting:** VLD04-D1 accepted Option A (local-first authoritative demo on presenter laptop in Mode B); remote deployment is deferred and not required for the current candidate. Parent VLD-04 remains blocked only on independent teammate reproduction (VLD-04B).
...
**Current state — TEMP-R1 / ADR 0006 and PUX-12A / ADR 0007 (2026-09-23):** PUX-11A real single-batch frontend lookup is integrated via PR #46. IGR-05B backend collection serving is integrated via PR #49. TEMP-R1 was Human Integrated via PR #51, merge commit `d6b7df1bf833e0be806df77873ed4de9695ccc5a`. Under accepted Human Gate decisions TEMP-D1–D5 ([ADR 0006](./decisions/0006-source-time-semantics.md)), official sponsor timestamps are recognized as fixed source-local UTC+03 for the supplied training snapshot (`sponsor_pack/README.md` §4). Collection runtime normalizes naive source/API timestamps to source UTC+03 without wall-clock shift, normalizes aware API timestamps preserving physical instant, and returns effective replay boundaries with `+03:00`. The collection endpoint applies the corrected D2R default `2025-11-29T00:00:00+03:00 <= dispatch_datetime < 2025-12-01T00:00:00+03:00` only when both bounds are omitted, preserving the same pinned 18-batch cohort; explicit paired bounds remain configurable. It filters held-out candidates by dispatch window and optional facility, maps/scores the whole matching cohort, sorts by `risk.score DESC, batch_id ASC`, then slices by limit/offset. It returns the collection envelope and fails closed on mapping/scoring errors. Under accepted FE-D1–FE-D6 ([ADR 0007](./decisions/0007-frontend-ranked-queue-ux.md)), PUX-12A / PR #52 consumes the collection for a queue-first frontend and retains single-batch lookup. No production deployment is claimed.
...
The initial runtime slice (offline artifact generation/validation, runtime loading, lifespan/provider, single-batch GET, health reconciliation and focused verification) is implemented in code and integrated via RBS-01 / PR #42. PUX-11A / PR #46 implemented frontend consumption of the real single-batch route. IGR-05B / PR #49 implemented backend multi-batch/replay serving, facility filtering, ranking and pagination. TEMP-R1 / PR #51 integrated UTC+03 source-time semantics. PUX-12A frontend collection queue/presentation was Human Integrated through PR #52. VLD-03 integration was Human Integrated through PR #53. VLD-04A operational evidence is recorded in this changeset; VLD-05A claim audit was Human Integrated through PR #54; database, learned/recommendation/deterioration engines, new confidence/minimum policy, comparability, background retraining, hot reload, and deployment remain deferred. If a valid canonical input cannot legitimately be assessed by the existing baseline: **STOP — HUMAN DECISION REQUIRED**. Dataset-backed simulation is not the synthetic fixture; fixture semantics remain unchanged and simulation=false remains outside HG-R5.
...
| TEMP-R1 / ADR 0006 | ACCEPTED / IMPLEMENTED / HUMAN INTEGRATED | [ADR 0006](./decisions/0006-source-time-semantics.md) | TEMP-D1–D5 accepted 2026-09-23; [PR #51](https://github.com/Slave-of-Skynet/training_agrifood/pull/51), merge commit `d6b7df1bf833e0be806df77873ed4de9695ccc5a` |
| PUX-12A / ADR 0007 | ACCEPTED / IMPLEMENTED / HUMAN INTEGRATED | [ADR 0007](./decisions/0007-frontend-ranked-queue-ux.md) | FE-D1–FE-D6 accepted 2026-09-23; [PR #52](https://github.com/Slave-of-Skynet/training_agrifood/pull/52), merge commit `b88e81a700188d9dd7dc95d9be757a6b6b81f900` |
| VLD-03 | ACCEPTED / EVIDENCE COMPLETE / HUMAN INTEGRATED | [VLD-03 end-to-end integration](./recon/VLD-03-end-to-end-integration.md) | Verified on `b88e81a700188d9dd7dc95d9be757a6b6b81f900`; Human Integrated through [PR #53](https://github.com/Slave-of-Skynet/training_agrifood/pull/53), merge commit `4363e372633eaeb09b9708c31923701852873455` |
| VLD-04 | OPERATIONAL EVIDENCE RECORDED (VLD-04A); PARENT GATE BLOCKED: MISSING EVIDENCE | [VLD-04A runtime and demo readiness](./recon/VLD-04-runtime-demo-readiness.md) | Verified on `4363e372633eaeb09b9708c31923701852873455`; clean worktree, runbook, fallback ladder, and VLD04-D1 accepted local-first; independent teammate reproduction pending (VLD-04B) |
| VLD-05A | ACCEPTED / HUMAN INTEGRATED | [VLD-05A claim and evidence audit](./recon/VLD-05A-claim-evidence-audit.md) | Accepted by Vladimir; [PR #54](https://github.com/Slave-of-Skynet/training_agrifood/pull/54), merge commit `a177c20dad4ed74be2dd5f46120ecc73767b8dcf` |
...
| IGR-05B | **ACCEPTED / IMPLEMENTED / HUMAN INTEGRATED** through [PR #49](https://github.com/Slave-of-Skynet/training_agrifood/pull/49) | Backend collection serving, shared response contract, replay window, facility filter, global ranking, limit/offset and fail-closed behavior implemented; PUX-12A frontend queue consumption was Human Integrated through PR #52 |
| TEMP-R1 / ADR 0006 | **COMPLETE / ACCEPTED / HUMAN INTEGRATED** through [PR #51](https://github.com/Slave-of-Skynet/training_agrifood/pull/51) | Reconciles collection serving with continuous UTC+03 sponsor timestamps; preserves D2R 18-batch cohort |
| PUX-12A / ADR 0007 | **ACCEPTED / IMPLEMENTED / HUMAN INTEGRATED** through [PR #52](https://github.com/Slave-of-Skynet/training_agrifood/pull/52) | Queue-first frontend consumes `GET /api/v1/assessments` under FE-D1–FE-D6 |
| VLD-03 | **ACCEPTED / EVIDENCE COMPLETE / HUMAN INTEGRATED** | Integrated through [PR #53](https://github.com/Slave-of-Skynet/training_agrifood/pull/53), merge commit `4363e372633eaeb09b9708c31923701852873455` |
| VLD-04 | **OPERATIONAL EVIDENCE RECORDED (VLD-04A); PARENT BLOCKED: MISSING EVIDENCE** | [Operational demo readiness evidence](./recon/VLD-04-runtime-demo-readiness.md) on PR #53 merge base; VLD04-D1 accepted (local-first); independent teammate reproduction pending for VLD-04B |
| VLD-05A | **ACCEPTED / HUMAN INTEGRATED** | Integrated through [PR #54](https://github.com/Slave-of-Skynet/training_agrifood/pull/54), merge commit `a177c20dad4ed74be2dd5f46120ecc73767b8dcf` |
...
| VLD-05 | **BLOCKED** on final claims prerequisites | VLD-05A audit integrated; final pitch narrative and deck consistency audit remain separate |
...
3. **Single-Batch Frontend and Ranked Queue Integrated:** PUX-11A (PR #46) integrated real single-batch assessment lookup through `GET /api/v1/assessments/{batch_id}` with HTTP 404/409/503 feedback. PUX-12A consumes `GET /api/v1/assessments` when analytics is ready, renders the backend-ordered replay queue, and reuses the selected collection assessment for detail. It was Human Integrated through PR #52.
...
8. **Local-First Demo Selected; Remote Deployment Deferred:** VLD04-D1 accepted Option A (local-first authoritative demo in Mode B on presenter machine). Remote cloud deployment is deferred and not required for the current candidate. Authoritative runbooks, verification scripts, and fallback paths are documented (VLD-04A); closing parent VLD-04 remains blocked on independent teammate reproduction (VLD-04B).
